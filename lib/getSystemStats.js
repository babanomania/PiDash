...
export default async function getSystemStats() {
  ...
  return {
    ...
    cpu: {
      temperature: cpuTemperature.main ? parseInt(cpuTemperature.main) : -1,
      usage: parseInt(currentLoad.currentLoad),
    },
    mem: {
      usage: parseInt((mem.active / mem.total) * 100),
      freeMB: parseFloat(mem.free / (1024 * 1024)).toFixed(0),
      usedMB: parseFloat(mem.active / (1024 * 1024)).toFixed(0),
    ...
    swap: {
      ...
      usedMB: parseFloat(
        (parseInt(mem.swaptotal) - parseInt(mem.swapfree)) / (1024 * 1024)
      ).toFixed(0),
